...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!whatsappSection || !hero) return;
  ...
  function toggleStickyWhatsApp() {
    ...
    const threshold = -300; // threshold in pixels for smoother transition (negative for more delay)

    if (heroBottom <= threshold && !isSticky) {
      whatsappSection.classList.add('fixed');
      isSticky = true;
    } else if (heroBottom > threshold && isSticky) {
  ...
  window.addEventListener('scroll', toggleStickyWhatsApp, { passive: true });
  toggleStickyWhatsApp(); // initial check
});
